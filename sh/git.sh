...
function gstash() {
    ...
    local selection=$(eval "$cmd" | fzf --preview "$preview" --height 80%)
    if [ -n "$selection" ]; then
        local stash_id=$(echo "$selection" | cut -d: -f1)
...
function grebase() {
    local commits
    commits=$(git log --oneline -n 50 --color=always | fzf --ansi --multi --preview 'git show --color=always {1}' | cut -d' ' -f1)
    if [ -n "$commits" ]; then
